...
This fork keeps `jcwillox/hassio-rclone-backup` behavior but adds optional/required rclone RC Web UI authentication for the Home Assistant add-on.

Changes in this fork:

- adds add-on options `rc_auth_enabled`, `rc_user`, and `rc_pass`;
- starts `rclone rcd` with `--rc-user/--rc-pass` when auth is enabled;
- removes the upstream prebuilt `image:` reference so Home Assistant can build this fork locally unless fork images are published later.

Upstream: https://github.com/jcwillox/hassio-rclone-backup
