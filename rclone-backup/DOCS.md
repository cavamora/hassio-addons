...
### Rclone Web UI authentication

This fork adds optional rclone RC/Web UI authentication. With the default fork configuration, authentication is enabled and `rc_pass` must be changed from `changeme` before the Web UI starts.

```yaml
rc_auth_enabled: true
rc_user: admin
rc_pass: use-a-real-password-here
```

Set `rc_auth_enabled: false` only if you intentionally want the upstream behavior (`--rc-no-auth`) and will rely solely on Home Assistant Ingress/network isolation.
