...
Hermes can work with HAOS `/media` and `/share` as explicit read/write mounts. The wrapper grants its unprivileged service user access at those two mount roots without recursively changing existing files; other add-ons' private volumes stay read-only.
